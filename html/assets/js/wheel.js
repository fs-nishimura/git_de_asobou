...
function SECTION_INIT() {
    ...
    TEXT_SECTION.each(function() {
        if (TEXT_SECTION.index(this) == 0) {
            ...
            $(this).find(".vertical_area").css({
                top: $(this).find(".vertical_area").height() - 300
            });
        } else {
            $(this).height($(this).find(".vertical_area").height() * 3);
            $(this).find(".vertical_area").css({
                top: $(this).find(".vertical_area").height() + 200
            });
        }
    });
        ...
        if (WHEEL_GRAND_TOTAL > WHEEL_CONTENT_H) {
            ...
            if (SECTION_NUM == 1 || SECTION_NUM == 2 || SECTION_NUM == 3) {
                ...
                if (VERTICAL_AREA.eq(SECTION_NUM - 1).offset().top < WINH) {
                    VERTICAL_AREA.eq(SECTION_NUM - 1).find(".txt,.tit").css({
                        opacity: 1
                    });
                    var VERTICAL_AREA_RATIO = VERTICAL_AREA.eq(SECTION_NUM - 1).offset().top / WINH;
                    VERTICAL_AREA.eq(SECTION_NUM - 1).find(".tit").css({
                        "margin-top": -(VERTICAL_AREA_RATIO * 200) + 80 + "%"
                    });
                ...
                $(".bgimg").css({
                    "margin-top": -(Math.floor((WHEEL_GRAND_TOTAL - WHEEL_SECTION_T[3]) / WHEEL_SECTION_H[3] * 100) * .4) + "%"
                });
            }
        }
    } else {
...
function SECTION_CHANGE(SECTION_NUM) {
    ...
    if (WHEEL_FLG == false) {
        WRAP.stop(true, false).animate({
            ...
            WHEEL_FLG = true;
            FADEOUT_FLG = false;
            $(".bgimg ul").stop(false, false).animate({
                opacity: "1"
            }, 300);
        });
    }
